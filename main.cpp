...
double ComputeValue(double a, double b, string expOperator)
{
	if (expOperator == "+")
		return (a + b);
	else if (expOperator == "-")
...
double ComputeReversePolishNotation(string input)
{
	...
	for (string s : elements)
	{
		if (IsOperator(s))
		{
			...
			double b = stod(resulting.top());
			resulting.pop();
			double a = stod(resulting.top());
			resulting.pop();
			double value = ComputeValue(a, b, s);
